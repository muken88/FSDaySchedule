...
public var IS_CREATE_ENABLED: Bool = true;

@objc open class ScheduleObjcModel: NSObject {
    @objc open var id: Int = 0
    @objc open var sId: String = ""
    @objc open var name: String = ""
    @objc open var start: CGFloat = 0
    @objc open var end: CGFloat = 0
    @objc open var startStr: String = ""
    @objc open var endStr: String = ""
    @objc open var isOverDay: Bool = false
}

class ScheduleModel: FSDayScheduleViewItemRepresentable {
    var id: Int = 0
    var sId: String = ""
    var name: String = ""
    var isOverDay: Bool = false
    var timeInfo: FSScheduleTimeInfo = .zero
}

/// 显示一天的日程安排视图,以时间轴格式,从每天的0点到第二天的0点,展示一天的日程安排
@objc open class FSDayScheduleView: UIView {
    
    @objc open var isCreateEnabled: Bool {
        get {
            return IS_CREATE_ENABLED
        }
        set {
            IS_CREATE_ENABLED = newValue
        }
    }
    
    @objc open var datasourceObjc: [ScheduleObjcModel] {
        get {
            return _datasourceObjc
        }
        set {
            _datasourceObjc = newValue
            //
            var models: [ScheduleModel] = []
            for oldModel in newValue {
                let model = ScheduleModel()
                model.id = oldModel.id
                model.sId = oldModel.sId
                model.name = oldModel.name
                model.timeInfo = FSScheduleTimeInfo(begin: oldModel.start, end: oldModel.end)
                model.isOverDay = oldModel.isOverDay;
                models.append(model)
            }
            _datasource = models.sorted(by: { (l, r) -> Bool in
    ...
    public var datasource: [FSDayScheduleViewItemRepresentable] {
        get {
            return _datasource
        }
        set {
    ...
    public weak var delegate: FSDayScheduleViewDelegate?
    
    @objc open weak var delegateObjc: FSDayScheduleViewDelegateObjc?
    
    // MARK: - private
    var _datasource: [FSDayScheduleViewItemRepresentable] = []
    
    open var _datasourceObjc: [ScheduleObjcModel] = []
    
    private lazy var scrollView: UIScrollView = {
        let scrollView = UIScrollView()
        scrollView.contentInset = FSDayScheduleView.ScrollViewContentInset
        //添加基尺view
        scrollView.addSubview(baselineView)
        baselineView.snp.makeConstraints { (maker) in
            maker.edges.equalTo(0)
            maker.width.equalToSuperview()
            maker.height.equalTo(FSDayScheduleView.ScrollContentViewHeight)
        }
        
        //添加日程view的父视图
        scrollView.addSubview(scheduleItemSuperView)
        scheduleItemSuperView.snp.makeConstraints { (maker) in
            maker.edges.equalTo(FSDayScheduleView.ItemSuperViewEdgeInsets)
        }
        
        //添加时间指示view,第一次添加会自动设置frame
        scrollView.addSubview(timeIndicatorView)
        timeIndicatorView.isHidden = true
    ...
    let baselineView = FSDayScheduleBaseLineView()
    ///当前时间指示器
    private let timeIndicatorView = FSDayScheduleTimeIndicatorView()
    ///所有日程Item的父视图
    let scheduleItemSuperView = UIView()
    ///保存具体日程view的数组,用于添加日程之前清空之前的view
    var scheduleViews: [FSDayScheduleItemView] = []
    
    // MARK: - datas
    ///已布局标志位
    private var layouted = false
    ///当前可编辑类型view,手势拖动时就是处理该view,可以是已存在的日程事项,也可以是新建日程view
    var currentEditableView: FSScheduleEditableView?
    ///触摸手势开始时,手指的位置
    ...
    ///编辑view触摸开始时的时间数据
    var touchBeginEditViewTimeData: FSScheduleTimeInfo = .zero
    ///触摸过程中临时保存的增量时间数据
    var touchMovingEditViewTimeData: FSScheduleTimeInfo = .zero
    ///编辑拖动手势
...
// MARK: - 手势代理
extension FSDayScheduleView: UIGestureRecognizerDelegate {
...
// MARK: - gesture even
private extension FSDayScheduleView {
    ...
    /// 设置创建日程的点击手势事件
    private func setupCreateScheduleViewAction(_ view: FSScheduleEditableView) {
        view.tapAction = { [unowned self] in
            let timeInfo = view.timeData
            let begin = FSScheduleTimeInfo.separateTime(timeInfo.begin)
            let startStr = String(format: "%02d:%02d:00", begin.hourValue,begin.minuteValue)
            let end = FSScheduleTimeInfo.separateTime(timeInfo.end)
            let endStr = String(format: "%02d:%02d:00", end.hourValue,end.minuteValue)
            self.delegateObjc?.dayScheduleView?(startStr: startStr, endStr: endStr)
            self.delegate?.dayScheduleView(self, createNewItemWith: view.timeData)
        }
    }
    
    /// 添加创建日程view
    private func createPlanView() -> FSScheduleEditableView {
        let createPlanView = FSScheduleEditableView(type: .add)
        setupCreateScheduleViewAction(createPlanView)
        scheduleItemSuperView.addSubview(createPlanView)
        createPlanView.frame = CGRect(x: 0, y: 0, width: self.itemViewMaxWidth, height: FSDayScheduleView.OneHourHeight)
        ...
        if self.currentEditableView != nil {
            ...
            if self.currentEditableView?.frame.contains(touchPosition) ?? false {
                ...
                if let itemView = self.currentEditableView?.associateItemView {
                    //只有编辑已有的日程,才需要走这里
                    let timeInfo: FSScheduleTimeInfo = self.currentEditableView!.editMaskView.timeData
                    let model: FSDayScheduleViewItemRepresentable = itemView.model
                    self.delegate?.dayScheduleView(self, editItem: model, timeInfo: timeInfo)
                    //编辑
                    let begin = FSScheduleTimeInfo.separateTime(timeInfo.begin)
                    let startStr = String(format: "%02d:%02d:00", begin.hourValue,begin.minuteValue)
                    let end = FSScheduleTimeInfo.separateTime(timeInfo.end)
                    let endStr = String(format: "%02d:%02d:00", end.hourValue,end.minuteValue)
                    let sModel = ScheduleObjcModel()
                    sModel.id = model.id
                    sModel.sId = model.sId
                    sModel.name = model.name
                    sModel.start = timeInfo.begin
                    sModel.end = timeInfo.end
                    sModel.startStr = startStr
                    sModel.endStr = endStr
                    self.delegateObjc?.dayScheduleView?(self, editItem: sModel)
                } else {
            ...
            let beginTime = min(CGFloat(FSDayScheduleView.HourAmoutOneDay - 1), self.propositionalHourValueFromPosition(gesture.location(in: gesture.view).y))
            self.currentEditableView?.becomeEditStatus(timeData: FSScheduleTimeInfo(begin: beginTime, end: beginTime + 1))
            self.scheduleItemSuperView.bringSubviewToFront(self.currentEditableView!)
    ...
    private func autoScrollWith(direction: VKPlanTimelineViewAutoScrollDirection) {
        ...
        let step = FSDayScheduleView.OneHourHeight * 0.5
        switch direction {
        case .top:
            y -= step
            contentOffset.y = max(0 - FSDayScheduleView.ScrollViewContentInset.top, y)
            self.scrollView.setContentOffset(contentOffset, animated: false)
            
        case .bottom:
            y += step
            contentOffset.y = min(y, self.scrollView.contentSize.height + FSDayScheduleView.ScrollViewContentInset.bottom - self.scrollView.height)
            self.scrollView.setContentOffset(contentOffset, animated: false)
        }
    }
    
    private func handleCurrentEditableViewWith(editEvenType: FSScheduleEditableEvenType, touchPosition: CGPoint, moveDirect: VKPlanTimelineViewPanGestureMoveDirect) {
        ...
        case .begin:
            ...
            guard timeLength >= FSDayScheduleView.MinPlanHourValue else {
                //时间小于半小时了,不允许
...
// MARK: - public func
public extension FSDayScheduleView {
    /// 清除当前编辑的日程view
...
internal extension FSDayScheduleView {
    ...
    static var ScrollContentViewHeight: CGFloat {
        return FSDayScheduleView.OneHourHeight * 24
    }
    
    /// 日程ItemView的最大宽度
    var itemViewMaxWidth: CGFloat {
        return self.bounds.width - FSDayScheduleView.ItemSuperViewEdgeInsets.horizontal
    }
}
...
internal extension FSDayScheduleView {
    ...
    func propositionalTopFromPosition(_ position: CGFloat) -> CGFloat {
        let value = propositionalHourValueFromPosition(position) * FSDayScheduleView.OneHourHeight
        return value
    }
    ...
    func propositionalHourValueFromPosition(_ position: CGFloat) -> CGFloat {
        ...
        if position > 24 * FSDayScheduleView.OneHourHeight {
            return 24
        }
        
        //将触摸的位置,计算出是第几小时
        let hourMinuteValue = FSDayScheduleView.hourValueFrom(position: position)
        let hourValue = hourMinuteValue.hourValue
        let minuteValue = hourMinuteValue.minuteFloatValue
        
        let minuteHeight = minuteValue * FSDayScheduleView.OneHourHeight
        //创建日程或者移动日程以15分钟为步长
        let minuteStep = FSDayScheduleView.OneHourHeight / 4
        
        var index = 0
        var inCreateHourVallue: CGFloat = 0
        while inCreateHourVallue <= FSDayScheduleView.OneHourHeight {
