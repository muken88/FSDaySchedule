//
//  FSDayScheduleView+LayoutItem.swift
//  FSDaySchedule
//
...
/// 构造树的节点
private class FSDayScheduleItemNode {
    ///每个节点最小的时间长度,根据最小文本高度计算
    static var minTimeLength: CGFloat = {
        let minItemHeight = FSDayScheduleItemView.MinItemHeight
        //将高度转换为小时
        return minItemHeight / FSDayScheduleView.OneHourHeight
    }()
    
    let data: FSDayScheduleItemView
    ///为了性能,这里保存一份开始和结束的时间
    let timeInfo: FSScheduleTimeInfo
    ///最大的结束时间,在自己和所有孩子节点中,获取最大的,在每次插入子节点时都更新
    var maxEnd: CGFloat
    ///孩子节点
    var children: [FSDayScheduleItemNode] = []
    ///层级,用于计算作为父节点时应该平分多少宽度
    var level: Int = 1
    ///父节点,用于插入子节点时更新父节点的level
    weak var parent: FSDayScheduleItemNode?

    init(_ data: FSDayScheduleItemView) {
        self.data = data
        //若结束时间与开始时间太短,这里要求设计一个最小高度,
        let endTime = max(FSDayScheduleItemNode.minTimeLength + data.model.timeInfo.begin, data.model.timeInfo.end)
        self.timeInfo = FSScheduleTimeInfo(begin: data.model.timeInfo.begin, end: endTime)
        self.maxEnd = endTime
    }
    ...
    func contains(_ node: FSDayScheduleItemNode, useMaxEndTime: Bool = false) -> Bool {
...
///   - targetNode: 被插入的节点
@discardableResult
private func insertNodeFrom(header: FSDayScheduleItemNode, targetNode: FSDayScheduleItemNode) -> Bool {
...
internal extension FSDayScheduleView {
    ...
    func layoutSubItems() {
        ...
        _datasource.forEach {
            let itemView = FSDayScheduleItemView(model: $0)
    ...
    func setupItemViewTopHeight(_ itemView: FSDayScheduleItemView) {
        let beginHourValue = itemView.model.timeInfo.begin
        //限定每个item需要最小高度,
        let endHourValue = max(itemView.model.timeInfo.end, beginHourValue + FSDayScheduleItemNode.minTimeLength)
        let x: CGFloat = 0
    ...
    private func updateItemsFrame() {
        ...
        var itemNodeArray: [FSDayScheduleItemNode] = []
        var rootNode: FSDayScheduleItemNode?
        for itemView in self.scheduleViews {
            if rootNode == nil {
                //1.若rootNode为空,初始化一个节点并赋值给rootNode,继续下一轮循环
                rootNode = FSDayScheduleItemNode(itemView)
                itemNodeArray.append(rootNode!)
                continue
            }
            //2.初始化一个节点node,
            let node = FSDayScheduleItemNode(itemView)
            //2.1若rootNode能包含node
    ...
    ///   - maxWidth: 节点的最大宽度
    ///   - level: 节点的子孩子数
    private func setupNodeWidthFrom(node: FSDayScheduleItemNode, left: CGFloat, maxWidth: CGFloat, level: Int) {
        guard level > 1 else {
    ...
    private func setupItemViewAction(_ itemView: FSDayScheduleItemView) {
        ///点击item的动作
...
//                    self.clearCurrentEditView()
            ...
            //
            for scheduleView in self.scheduleViews {
                scheduleView.viewStatus = .normal
            }
            self.delegate?.dayScheduleView(self, didSelectItem: model)
            //点击
            let ScheduleObjcModel = ScheduleObjcModel()
            ScheduleObjcModel.id = model.id
            ScheduleObjcModel.sId = model.sId
            ScheduleObjcModel.name = model.name
            ScheduleObjcModel.start = model.timeInfo.begin
            ScheduleObjcModel.end = model.timeInfo.end
            self.delegateObjc?.dayScheduleView?(self, didSelectItem: ScheduleObjcModel)
        }
        
        itemView.editStatusChangeAction = { [weak self] itemView in
            guard let self = self else { return }
            self.clearCurrentEditView()
            self.currentEditableView = FSScheduleEditableView(type: .edit, associateItemView: itemView)
            self.currentEditableView?.frame = CGRect(x: 0, y: 0, width: self.itemViewMaxWidth, height: FSDayScheduleView.OneHourHeight)
        ...
        itemView.editStatusWillChangeAction = { [weak self] model in
            guard let self = self else { return }
            //点击
            let ScheduleObjcModel = ScheduleObjcModel()
            ScheduleObjcModel.id = model.id
            ...
            ScheduleObjcModel.start = model.timeInfo.begin
            ScheduleObjcModel.end = model.timeInfo.end
            self.delegateObjc?.dayScheduleView?(self, willEditItem: ScheduleObjcModel)
        }
    }
}
