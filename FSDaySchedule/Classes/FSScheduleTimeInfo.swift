...
public struct FSScheduleTimeInfo: Equatable, CustomDebugStringConvertible {
    public var debugDescription: String {
        let beginTime = FSScheduleTimeInfo.separateTime(begin)
        let endTime = FSScheduleTimeInfo.separateTime(end)
        return "\n开始:\(beginTime.hourValue):\(beginTime.minuteValue)\n结束:\(endTime.hourValue):\(endTime.minuteValue)"
    }
    
    public static var zero: FSScheduleTimeInfo {
        return FSScheduleTimeInfo(begin: 0, end: 0)
    }
    ...
    public func contains(otherTimeData: FSScheduleTimeInfo) -> Bool {
...
internal extension CGFloat {
    ...
    var verticalPosition: CGFloat {
        return FSDayScheduleView.OneHourHeight * self
    }
}
